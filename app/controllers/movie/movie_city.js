      ...
      if(cityCategoryName) {
        CityCategory.findOne({name:cityCategoryName}, function(err,_cityCategory) {
          ...
          if(_cityCategory) {
            ...
            _oldCityProgramme.cityCategories = _oldCityProgramme.cityCategories.concat(newCityCategory._id);
      ...
      City.findOne({name:cityName}, function(err,_city) {
        ...
        if (_city) {
          ...
          if(typeof cityCategoryId === 'string') {
            cityCategoryArray = cityCategoryArray.concat(cityCategoryId);
          }else {
          ...
          for(var i = 0; i < cityCategoryArray.length; i++) {
            CityCategory.findById(cityCategoryArray[i], function(err, _oldCityCategory) {
              if(_oldCityCategory) {
                newCity.cityCategories = newCity.cityCategories.concat(_oldCityCategory._id);
                _oldCityCategory.cities = _oldCityCategory.cities.concat(newCity._id);
    ...
    if (cinemas) {
      City.findById(cityId,function(err,_city) {
        ...
        if(_city.cinemas.indexOf(cinemas) === -1) {
          _city.cinemas = _city.cinemas.concat(cinemas);
  ...
  if(id) {
    ...
    CityCategory.findById(id, function(err,_cityCategory) {
      ...
      if(typeof _cityCategory.cities === 'string') {
        cityArray = cityArray.concat(_cityCategory.cities);
      }else {
