...
exports.save = function(req,res) {
  ...
  // 如果数据已存在，则更新相应修改的字段
  if(id) {
    Movie.findById(id,function(err,_movie) {//console.log('bug')
      if(err) {
        console.log(err);
      }
      // 如果修改电影分类
      if (movieObj.category.toString() !== _movie.category.toString()) {
        ...
        Category.findById(movieObj.category,function(err,_newCat) {
          ...
          _newCat.movies = _newCat.movies.concat(id);
        ...
        newMovie.save(function(err,_newMovie) {
          ...
          if(categoryId) {
            Category.findById(categoryId,function(err,_category) {
              ...
              console.log('before');console.log(_category.movies);
                console.log('id');console.log(_newMovie._id);
              //_category.movies.push(_newMovie._id);//$pushAllMongo 3.6.2（或3.6.x +中的任何较新版本）不再支持该运算符
                _category.movies = _category.movies.concat(_newMovie._id);
                console.log('after');console.log(_category.movies);
